...
1904_TheNumberofFullRoundsYouHavePlayed
...
public:
    int numberOfRounds(string startTime, string finishTime) {
        ...
        int ST_HH = (startTime[0]-'0')*10 + (startTime[1]-'0');
        int ST_MM = (startTime[3]-'0')*10 + (startTime[4]-'0');
        int FT_HH = (finishTime[0]-'0')*10 + (finishTime[1]-'0');
        int FT_MM = (finishTime[3]-'0')*10 + (finishTime[4]-'0');
        
        if(ST_HH>FT_HH || ST_HH==FT_HH && ST_MM>FT_MM) 
            FT_HH += 24;        
		
        ST_MM += (ST_MM%15==0 ? 0: 15-ST_MM%15);
        FT_MM -= (FT_MM%15==0 ? 0 : FT_MM%15);
		
        int time1 = ST_HH*4 + ST_MM/15, time2 = FT_HH*4 + FT_MM/15;
